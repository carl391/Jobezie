...
class LinkedInService:
    ...
    def analyze_profile(
        ...
        experience_analysis = cls._analyze_experience(profile_data.get("experience", []))
        ...
        skills_analysis = cls._analyze_skills(
            profile_data.get("skills", []), profile_data.get("target_role", "")
        )
    ...
    def generate_headline(
        ...
        keywords = cls.INDUSTRY_KEYWORDS.get(
            (industry or "").lower(), ["professional", "experienced", "dedicated"]
        )
    ...
    def generate_summary(
        ...
        return {
            ...
            "sections": [{"name": name, "content": text} for name, text in sections],
            "tips": [
    ...
    def _analyze_summary(cls, summary: str) -> Dict:
        ...
        if any(
            ...
            for phrase in ["reach out", "connect", "let's chat", "contact"]
        ):
            score += 10
        else:
