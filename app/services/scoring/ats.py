...
ACTION_VERBS = [
    ...
    'built', 'boosted',
    'consolidated', 'created', 'cultivated',
    'delivered', 'developed', 'drove',
    'earned', 'enabled', 'engineered', 'established', 'exceeded', 'expanded',
    'founded',
    'generated', 'grew',
    'headed',
    'implemented', 'improved', 'increased', 'initiated', 'innovated',
    'launched', 'led',
    'managed', 'maximized', 'modernized',
    'negotiated',
    'optimized', 'orchestrated', 'outperformed', 'overhauled',
    'pioneered', 'produced',
    'reduced', 'redesigned', 'reorganized', 'resolved', 'revamped',
    'saved', 'scaled', 'secured', 'simplified', 'spearheaded', 'streamlined',
...
def calculate_ats_score(
    ...
    completeness, complete_feedback, weak_sections = _score_completeness(resume_text, parsed_sections)
...
def _score_achievements(resume_text: str) -> Tuple[int, List[str]]:
    ...
    result_keywords = ['resulting', 'achieved', 'led to', 'which', 'saving', 'generating', 'improving']
    ...
    if len(bullets) > 0:
        ...
        if result_rate >= 0.5:
            ...
            feedback.append('Add results and outcomes to your bullet points (e.g., "...resulting in 25% increase")')
...
def _score_formatting(resume_text: str, parsed_sections: Optional[Dict]) -> Tuple[int, List[str]]:
    ...
    common_headers = ['experience', 'education', 'skills', 'summary', 'objective', 'work history']
...
def _score_progression(resume_text: str, parsed_sections: Optional[Dict]) -> Tuple[int, List[str]]:
    ...
    progression_terms = [
        'promoted', 'advanced', 'senior', 'lead', 'manager', 'director',
        'head of', 'principal', 'chief', 'vp', 'vice president'
    ]
    resume_lower = resume_text.lower()
...
def _score_completeness(
    ...
    section_checks = {
        ...
        'summary': any(s in resume_text.lower() for s in ['summary', 'objective', 'profile', 'about']),
        'experience': any(s in resume_text.lower() for s in ['experience', 'employment', 'work history']),
