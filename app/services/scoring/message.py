...
def calculate_message_quality(
    ...
    for item in words_feedback + personal_feedback + metrics_feedback + cta_feedback + tone_feedback:
        if item.startswith('✓'):
...
def _score_word_count(message_text: str, message_type: str) -> Tuple[int, int, List[str]]:
    ...
    if min_words <= word_count <= max_words:
        ...
        feedback.append(f'Add {deficit} more words for optimal length ({min_words}-{max_words} words)')
    else:
        # Too long
        excess = word_count - max_words
        score = max(0, 100 - (excess * 2))
        if word_count > 150:
            feedback.append(f'Reduce to under 150 words for 22% higher response rate (currently {word_count})')
        else:
...
def _score_personalization(
    ...
    if not elements_found:
        feedback.append('Add personalization - reference something specific about them or their company')
...
def _score_metrics(message_text: str) -> Tuple[int, List[str]]:
    ...
    if metrics_found >= 3:
        ...
        score = 20
        feedback.append('Include specific numbers and metrics to stand out (40% more interview callbacks)')
...
def _score_tone(message_text: str, message_type: str) -> Tuple[int, List[str]]:
    ...
    desperate = ['i really need', 'please help', 'desperate', 'any job', 'anything available']
